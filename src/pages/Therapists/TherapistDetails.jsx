...
const TherapistDetails = () => {
    ...
    useEffect(()=>{
        if(therapists && therapists.length){
            const therapist=therapists.find((therapist)=>therapist._id===therapistsId);
            setTherapistDetail(therapist);
        }
    },[therapists, therapistsId]);

    const { Name,Image, Specialty,Degree,Rating ,email, AvailableTime} = therapistDetails;
    return (
      ...
      <div className="card-details">
        ...
        <Link to='/bookappointment' ><button className="btn btn-primary">Book Appointment</button></Link>
